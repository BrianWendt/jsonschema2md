...
function ejsRender(template, ctx) {
  console.log('ejsRender', template);
...
/**
 * Generates a {Schema}.md file
 * @param {string} filename - name of the schema file
 * @param {object} schema
 * @param {string} schemaPath - schema base directory
 * @param {string} outDir - output directory
 * @params {map} dependencyMap
 * @param {map} docs - a map of documentation links for headers
 * @param {string} templatesDir
 */
const generateMarkdown = function(filename, schema, schemaPath, outDir, dependencyMap, docs, templatesDir) {
  var ctx = {
    schema: schema,
    _: _,
    validUrl: validUrl,
  ...
  multi = multi.map(([ template, context ]) => {
    var templatePath = path.join(__dirname, '../templates/md/' + template);
    const templateCustom = path.join(templatesDir, template);
    try {
        if(fs.existsSync(templateCustom)){
            templatePath = templateCustom;
        }
    } catch(err){
