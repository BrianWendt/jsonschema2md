 ...
 */
Schema.process = function(schemaMap, schemaPath, docDir, schemaDir, metaElements, readme, docs, templatesDir) {
  smap=schemaMap;
  ...
  return Promise.mapSeries(keys, schemaKey => {
    ...
    return Schema.getExamples(schemaMap[schemaKey].filePath, schema)
      ...
      .then(allSchema => {
        ...
        const outputTasks = [ markdownWriter(schemaMap[schemaKey].filePath, object.mSchema, schemaPath, docDir, object.dep, docs, templatesDir) ];
        if (schemaDir !== '') {
    ...
    if (readme) {
      ...
      return readmeWriter(markdowns, schemaMap, docDir, schemaPath, templatesDir);
    } else {
