 ...
 */
const generateReadme = function(paths, schemas, out, base, templatesDir) {
  ...
  var readmeTemplate = path.join(__dirname, '../templates/md/readme.ejs');
  const readmeTemplateCustom = path.join(templatesDir, 'readme.ejs');
  try {
    if(fs.existsSync(readmeTemplateCustom)){
        readmeTemplate = readmeTemplateCustom;
    }
  } catch(err){
      
  }
  
  return pejs.renderFileAsync(readmeTemplate, ctx, { debug: false }).then(str => {
